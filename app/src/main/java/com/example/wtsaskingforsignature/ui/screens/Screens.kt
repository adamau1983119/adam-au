	...
	Column(Modifier.fillMaxSize().padding(16.dp)) {
		...
		if (rolls.value.isNotEmpty()) {
			...
			Text(
				...
				color = if (isValidResult) MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.onSurfaceVariant,
		...
		Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
			WtsWhiteButton(
				text = "直接查看籤文", 
				onClick = { navigateWithThrottle(Routes.content(id)) }, 
				modifier = Modifier.weight(1f),
		...
		Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
			WtsWhiteButton(
				text = "返回抽籤", 
				onClick = { navigateWithThrottle(Routes.DIRECT_DRAW) },
				enabled = !isNavigating.value
			)
			WtsWhiteButton(
				text = "回首頁", 
				onClick = { navigateWithThrottle(Routes.HOME) },
				enabled = !isNavigating.value
			)
		}
	}
